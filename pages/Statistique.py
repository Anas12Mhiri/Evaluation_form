...
if file is not None:
    ...
    st.markdown("<h3 style='text-align: center; color: #4CAF50;'>📈 Répartition des priorités</h3>", unsafe_allow_html=True)
    m.repartition_priorite1()
    st.markdown("<h3 style='text-align: center; color: #4CAF50;'>📈 Répartition des catégories</h3>", unsafe_allow_html=True)
    m.repartition_categorie1()
    st.markdown("<h3 style='text-align: center; color: #4CAF50;'>📈 Distribution des tâches</h3>", unsafe_allow_html=True)
    m.terminées_vs_À_faire1()
    st.markdown("<h3 style='text-align: center; color: #4CAF50;'>📈 Distribution des priorités par statut</h3>", unsafe_allow_html=True)
    m.nbr_taches_par_statut_et_priorité1()
    statut_filtre = dataframe['Statut'].unique()
    dataframe = dataframe[dataframe['Statut'].isin(statut_filtre)]
    st.markdown("<h3 style='text-align: center; color: #4CAF50;'>📈 Votre Progrès</h3>", unsafe_allow_html=True)
    st.line_chart(dataframe.groupby('Date_echeance').size())
