...
if m.storage is not None and "Titre" in m.storage.columns:
    titre2_list = m.storage["Titre"].dropna().tolist()
else:
    titre2_list = []



titre2 = st.selectbox("Sélectionner titre", titre2_list if titre2_list else ["Aucune tâche disponible"])
if titre2 and titre2 != "Aucune tâche disponible":
    list_id = m.storage[m.storage["Titre"] == titre2]["ID"]
    id2 = st.selectbox("ID tâche", list_id)

    if st.button("Supprimer"):
        st.warning("Voulez-vous vraiment supprimer cette tâche ?")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Oui, supprimer définitivement"):
                m.supprimer_tache(id2)
                st.success("Tâche supprimée avec succès.")
        with col2:
            if st.button("Annuler"):
                st.success("Annulation confirmer.")
