...
def generer_id_suivant():
    try:
        if not os.path.exists("taches.csv") or os.path.getsize("taches.csv") == 0:
            return 1
        data = pd.read_csv("taches.csv")
...
class task:
    ...
    def est_echeance_imminente(self):
        ...
        return 0 <= (echeance_datetime - maintenant) <= timedelta(hours=24)
...
class taskManager: 
    ...
    def rechercher_tache_par_titre(self,t):
        result = [i for i in self.tasks if t.lower() in i.titre.lower()]
        return pd.DataFrame(result)
    def rechercher_tache_par_categorie(self,t):
        result = [i for i in self.tasks if i.categorie.lower() == t.lower()]
        return pd.DataFrame(result)
    def rechercher_tache_par_description(self,t):
        result = [i for i in self.tasks if t.lower() in i.description.lower()]
        return pd.DataFrame(result)
...
titre1 = st.selectbox("Sélectionner un titre", titre1_list if titre1_list else ["Aucune tâche disponible"])
if titre1 and titre1 != "Aucune tâche disponible":
    taches = m.storage[m.storage["Titre"] == titre1].iloc[0]
    list_id = m.storage[m.storage["Titre"] == titre1]["ID"]
    id1 = st.selectbox("ID",list_id)
    description1 = st.text_input("Description", value=taches["Description"])
    date_echeance1 = st.date_input("Date d'échéance", value=datetime.strptime(taches["Date_echeance"], "%Y-%m-%d").date())
    priorite1 = st.selectbox("Priorité",["faible", "moyenne", "élevé"], index=["faible", "moyenne", "élevé"].index(taches["Priorite"]))
    categorie1 = st.selectbox("Catégorie", ["travail", "études", "personnel", "santé"], index=["travail", "études", "personnel", "santé"].index(taches["Categorie"])
    )
    statut1 = st.selectbox("Statut", ["À faire", "Terminé"], index=["À faire", "Terminé"].index(taches["Statut"]))
    
    btn3 = st.button("Modifier")
    
    if btn3:
        m.modifier_tache(id1, "Description", description1)
        m.modifier_tache(id1, "Date_echeance", str(date_echeance1))
        m.modifier_tache(id1, "Priorite", priorite1)
        m.modifier_tache(id1, "Categorie", categorie1)
        m.modifier_tache(id1, "Statut", statut1)
        st.success(f"Tâche '{titre1}' modifiée avec succès.")
else:
    st.info("Aucune tâche disponible à modifier.")
